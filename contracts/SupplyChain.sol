...
address payable buyer;
  ...
  /* Create 4 events with the same name as each possible State (see above)
   ...
   event LogForSale(uint sku);
    ...
    event LogoShipped(uint sku);
    event LogoReceived(uint sku);
/* Create a modifer that checks if the msg.sender is the owner of the contract */
   ...
   */
   ...
   modifer forSale(uint _sku){
    require(items[_sku].state == State.ForSale);
    require(items[_sku].price != 0);
    require(items[_sku].seller != address(0));
    _;
  }
  modifer sold(uint _sku){require(items[_sku].state == State.Sold);_;}
  modifer shipped(uint _sku){require(items[_sku].state == State.Shipped);_;}
  modifer received(uint _sku){require(items[_sku].state == State.Received);_;}
  /// modifier forSale
  /// modifier sold
  /// modifier shipped
  /// modifier received
